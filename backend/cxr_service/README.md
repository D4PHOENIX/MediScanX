...
This service includes a multi-stage `Dockerfile` optimized with `uv` for fast, cached dependency installations. It leverages a non-root user (`mediscan`) for enhanced security and includes a healthcheck endpoint for orchestration liveness probes.

To build and run the Docker container locally:

```bash
docker build -t cxr-service .
docker run -p 8001:8001 cxr-service
```

The service is also integrated with a GitHub Actions CI pipeline (`.github/workflows/cxr-service.yml`) that triggers on pull requests to the `dev` and `main` branches. The pipeline runs a comprehensive test suite with mocked weights and thresholds, followed by a multi-stage Docker build dry-run and smoke test.
