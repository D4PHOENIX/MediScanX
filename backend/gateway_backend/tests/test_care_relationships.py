...
async def test_revoke_care_no_live_relationship_returns_404(auth_headers):
    ...
    response = client.post("/api/v1/care-relationships/revoke", headers=auth_headers, json=payload)
    print(response.text)
...
async def test_missing_or_invalid_bearer_token():
    ...
    # GET
    response_get = client.get("/api/v1/care-relationships")
    assert response_get.status_code == status.HTTP_401_UNAUTHORIZED
    
    # POST
    payload = {"relationship_id": "11111111-2222-3333-4444-555555555555"}
    response_post = client.post("/api/v1/care-relationships/revoke", json=payload)
    assert response_post.status_code == status.HTTP_401_UNAUTHORIZED
    
    # Invalid token GET
    response_get_invalid = client.get("/api/v1/care-relationships", headers={"Authorization": "Bearer invalid"})
...
async def test_get_zero_relationships_returns_200_empty_array(auth_headers):
    ...
    response = client.get("/api/v1/care-relationships", headers=auth_headers)
