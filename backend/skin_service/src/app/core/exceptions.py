"""Domain‑specific exceptions for the Skin Lesion microservice.
...
class SkinBaseException(Exception):
    """Base class for all Skin lesion domain exceptions.
    ...
    def __init__(
        ...
        """Initializes the SkinBaseException.
...
class ImageProcessingError(SkinBaseException):
...
class UnreadableImageFormatError(ImageProcessingError):
    """OpenCV or PIL could not read the supplied image file."""
    ...
    def __init__(
        ...
        """Initializes the UnreadableImageFormatError.
...
class InvalidImageDimensionError(SkinBaseException):
    ...
    def __init__(
        ...
        """Initializes the InvalidImageDimensionError.
...
class ModelInferenceError(SkinBaseException):
...
class SkinModelNotFoundError(SkinBaseException):
    ...
    def __init__(
        ...
        """Initializes the SkinModelNotFoundError.
...
class SkinEngineNotReadyError(SkinBaseException):
    """Skin engine hasn't finished initialising – 503 Service Unavailable."""

    def __init__(
        self,
        context: Optional[Dict[str, Any]] = None,
    ) -> None:
        """Initializes the SkinEngineNotReadyError.

        Args:
            context (Optional[Dict[str, Any]], optional): Additional context payload. Defaults to None.
        """
        super().__init__(
            message="Skin engine is still loading or unavailable.",
...
class ExceptionRegistry:
    ...
    def register_handlers(cls, app: FastAPI) -> None:
        ...
        """
        @app.exception_handler(SkinBaseException)
        async def _skin_handler(request: Request, exc: SkinBaseException) -> JSONResponse:
            """Handler for all SkinBaseException domain errors.

            Args:
                request (Request): The incoming HTTP request.
                exc (SkinBaseException): The raised domain exception.

            Returns:
                JSONResponse: A structured JSON response with error details.
            """
            logger.error(
                "Skin lesion domain error at %s: %s | context=%s",
                request.url.path,
                exc.message,
                exc.context,
            )
